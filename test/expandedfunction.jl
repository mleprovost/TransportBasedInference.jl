...
@testset "Test evaluation, gradient and hessian of expanded function Nx = 1 I" begin
    ...
    # For truncated basis
    ψtrunc_basis = evaluate_basis(f, ens, truncidx)
    dψtrunc_basis = grad_x_basis(f, ens, truncidx)
    d2ψtrunc_basis = hess_x_basis(f, ens, truncidx);
    ...
    # Verify grad_coeff
    dψcoeff  = grad_coeff(f, ens)
    dψcoefftrunc  = grad_coeff(f, ens, collect(1:2:size(idx,1)))
    ...
    # Verify hess_coeff
    d2ψcoeff  = hess_coeff(f, ens)
    d2ψcoefftrunc  = hess_coeff(f, ens, collect(1:2:size(idx,1)))

    @test norm(d2ψcoeff)<1e-10

    # Verify grad_coeff_grad_xd
    dψcoeffxd = grad_coeff_grad_xd(f, ens)
    dψcoeffxdtrunc = grad_coeff_grad_xd(f, ens, collect(1:2:size(idx,1)))

    @test norm(dψcoeffxd[:,1:2:end] - dψcoeffxdtrunc)<1e-10
    @test norm(dψcoeffxd - grad_xk_basis(f, Nx,1,ens))<1e-10
    @test norm(dψcoeffxdtrunc - grad_xk_basis(f, Nx,1,ens, f.idx[collect(1:2:size(idx,1)),:]))<1e-10

    # Verify hess_coeff_grad_xd
    d2ψcoeffxd = hess_coeff_grad_xd(f, ens)
    d2ψcoeffxdtrunc = hess_coeff_grad_xd(f, ens, collect(1:2:size(idx,1)))
