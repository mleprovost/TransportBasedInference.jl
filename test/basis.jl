...
@testset "Test defined families list" begin
    ...
    # B = BasisPhyHermite(10; scaled =false)
    # @test size(B,1)==11
    #
    # for i=1:size(B,1)
    ...
    #
    #
    # B = BasisPhyHermite(10; scaled = true)
    # @test size(B,1)==11
    #
    # for i=1:size(B,1)
    ...
    B = BasisProHermite(10; scaled =false)
    @test size(B,1)==11

    for i=1:size(B,1)
        @test B[i] == FamilyProHermite[i]
    end


    B = BasisProHermite(10; scaled = true)
    @test size(B,1)==11

    for i=1:size(B,1)
        @test B[i] == FamilyScaledProHermite[i]
    end
...
@testset "Test evaluation of the basis" begin
    # @testset "Test evaluation of the basis" begin
        ...
        B = BasisProHermite(5; scaled =false)
        m = size(B,1)

        Bx = B(x[1])
        for i=0:5
            @test abs(Bx[i+1] -  ProHermite(i)(x[1]))< 1e-8
        end


        B = BasisProHermite(5; scaled = true)
        m = size(B,1)

        Bx = B(x[1])
        for i=0:5
            @test abs(Bx[i+1] -  ProHermite(i; scaled = true)(x[1]))< 1e-8
        end
        #
        # Ne = 10
        # x = 0.5*randn(Ne)
...
@testset "Verify vander for basis: evaluation first and second derivatives" begin
    ...
    x = randn(Ne)
    #Test all the basis
    Btab=  [ BasisProHermite(5; scaled = false);
             BasisProHermite(5; scaled = true) ]
