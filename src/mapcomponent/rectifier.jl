...
export  Rectifier,
        ...
        LOG2, softplus, dsoftplus, d2softplus, invsoftplus,
...
# Softplus tools
softplus(x) = (log(1.0 + exp(-abs(LOG2*x))) + max(LOG2*x, 0.0))/LOG2
dsoftplus(x) = 1.0/(1.0 + exp(-LOG2*x))
d2softplus(x) = LOG2/(2.0*(1.0 + cosh(LOG2*x)))
invsoftplus(x) = min(log(exp(LOG2*x) - 1.0)/LOG2, x)
