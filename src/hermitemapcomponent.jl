...
function hess_negative_log_likelihood!(J, dJ, d2J, coeff, S::Storage{m, Nψ, k}, Hk::HermiteMapk{m, Nψ, k}, X::Array{Float64,2}) where {T <: Real, m, Nψ, k}
    ...
    if d2J != nothing
        ...
        d2J .= zeros(Nψ, Nψ)
        @inbounds for l=1:Ne
            @show issymmetric(reshape2_cacheintegral[l,:,:])
            # Exploit symmetry later
            for i=1:Nψ
                for j=1:Nψ
                ...
                d2J[i,j] -=  ( (S.ψoff[l,i]*S.dψxd[l,i]) * (S.ψoff[l,j]*S.dψxd[l,j])*
                                ...
                                grad_x(Hk.I.g, S.cache_g[l])^2*(S.ψoff[l,i]*S.dψxd[l,i])*(S.ψoff[l,j]*S.dψxd[l,j]))/Hk.I.g(S.cache_g[l])^2
