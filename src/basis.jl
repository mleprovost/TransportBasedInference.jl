...
struct Basis{m}
    f::Array{ProHermite,1}
    function Basis(f::Array{ProHermite,1})
...
function BasisProHermite(m::Int64; scaled::Bool = false)
    if scaled == false
        return Basis(FamilyProHermite[1:m+1])
    else
        return Basis(FamilyScaledProHermite[1:m+1])
    end
end

# function BasisPhyHermite(m::Int64; scaled::Bool = false)
#     if scaled == false
#         return Basis(FamilyPhyHermite[1:m+1])
#     else
#         return Basis(FamilyScaledPhyHermite[1:m+1])
#     end
# end
#
#
...
(F::Array{S,1})(x::T) where {S <: ParamFcn, T <: Real} = map!(fi->fi(x), zeros(T, size(F,1)), F)
...
@propagate_inbounds Base.setindex!(B::Basis{m}, v::ProHermite, i::Int) where {m} = setindex!(B.f,v,i)
