...
#Version with low-rank structure

"""
		seqassim(F::StateSpace, data::SyntheticData, J::Int64, ϵx::InflationType, algo::SeqFilter, X, Ny, Nx, t0::Float64, Loc::Localization)

Generic API for localized sequential data assimilation for any sequential filter of type `LowRankStochEnKF`.
"""
function seqassim(F::StateSpace, data::SyntheticData, J::Int64, ϵx::InflationType, algo::Union{LowRankStochEnKF}, X, Ny, Nx,t0::Float64)
...
@inbounds for i=1:length(Acycle)
	...
	# Compute the projection bases U and V
	H = FowardDiff.Jacobian(f.h(x,t). X[Ny+1:Ny+Nx,1])
	L = LinearTransform(X[Ny+1:Ny+Nx,:]; diag = true)
	svdH̃ = svd(algo.ϵy.σ\H*L.L; full = true)
	U = svdH̃.U[:,1:algo.ry]
	V = Matrix(svdH̃.Vt[1:algo.rx,:]')
    # Generate posterior samples
    X = algo(X, ystar, t0+i*algo.Δtobs-t0, U, V)
